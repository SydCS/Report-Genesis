 ...
 * 对于前端发来的请求，这个过滤器都会进行过滤，
 * 具体是咱们的前置拦截处理和后置拦截处理分别返回响应正常状态和添加跨域请求。
 */
@Component
@Log4j2
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    /**
     * 过滤器拦截请求的入口方法,所有请求都会进入该方法，验证token的正确性
     * 正确则继续往下请求，如果验证出错则返回错误信息，拒绝访问！
     * 返回true则允许访问
     *
     * @param request
     * @param response
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        //看看源码，调用了isLoginAttempt()方法
        if (this.isLoginRequest(request, response)) {
            //headers中没有相应的token字段
...
//        httpServletResponse.setStatus(HttpStatus.UNAUTHORIZED.value());
        ...
        Result result = ResultUtil.unauthorized("未登录");
